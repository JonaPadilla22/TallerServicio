import { TipoRefaccion } from "../interfaces/tipoRefaccion.interface";
...
class TypeRepairService {
    ...
    static getTypeRepairById = async (id: string) => {
        let [rows] = await connection.query('SELECT * FROM tipo_refaccion WHERE ID_MARCA = ?',[parseInt(id)]);
        let users = rows.map((r: any) => {
            return r;
        });
        return users;   
    ...
    static getTypeRepairStatus = async (id: string) => {
        let [rows] = await connection.query('SELECT estatus FROM tipo_refaccion WHERE ID_MARCA = ?',[id]);
    ...
    static updateTypeRepair = async (item: TipoRefaccion, id: string) => {       
        const responseUpdate = await connection.query('UPDATE tipo_refaccion SET ? WHERE ID_MARCA = ?', [item, id]);
